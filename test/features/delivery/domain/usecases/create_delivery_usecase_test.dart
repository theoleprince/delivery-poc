...
void main() {
  ...
  late CreateDeliveryUseCase useCase;

  setUp(() {
    repository = _MockDeliveryRepository();
    useCase = CreateDeliveryUseCase(repository);
  });

  test('délègue au repository et renvoie la livraison créée', () async {
    final DeliveryEntity delivery = _buildDelivery();
    final DeliveryEntity created = delivery.copyWith(id: 'delivery-1');

    when(
      () => repository.createDelivery(delivery),
    ).thenAnswer((_) async => Result<DeliveryEntity>.success(created));

    final Result<DeliveryEntity> result = await useCase(
      CreateDeliveryParams(delivery: delivery),
    );

    expect(result, Result<DeliveryEntity>.success(created));
    verify(() => repository.createDelivery(delivery)).called(1);
  });
}
